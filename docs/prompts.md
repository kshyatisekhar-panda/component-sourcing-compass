...
These are full prompts sent to a separate LLM call from inside a tool. Currently only `generate_compliance_report` uses one. The model is `anthropic/claude-sonnet-4.5` via OpenRouter, temperature `0.3`, max tokens `1500`.
...
Built per call from the verdict data. Pseudocode:
...
The verdict data is the same shape returned by `compliance_check` so the LLM gets fully cited findings to draw from.
...
### Compliance certificate user prompt
...
2. Show me a cost ranked breakdown for Atlas Compressor Model A.
3. How has the 3kW motor's price moved over the last year?
4. Where can I save money on Compressor Model A? Look at the biggest cost components and check Mouser.
5. Which sourcing changes have the biggest cross product impact?
6. Can we sell Atlas Compressor Model A in the EU?
