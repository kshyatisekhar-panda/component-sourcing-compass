  ...
  server.registerTool(
    ...
    async ({ component_id, product_id, market }) => {
      ...
      if (component_id) {
        ...
        const verdict = evaluate(component, market, ctx);
        return text({
      ...
      const verdicts = product.components.flatMap(({ component_id: id }) => {
        const c = bom.components.find((x) => x.id === id);
